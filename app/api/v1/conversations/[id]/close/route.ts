...
export async function POST(req: NextRequest, ctx: RouteCtx): Promise<Response> {
  ...
  const { data: visible, error: readError } = await supabase
    ...
    .select("id, organization_id, service_revision")
    .eq("id", id)
  ...
  if (error)
    return fail(
      error.code === "40001" ? "conflict" : "internal_error",
      error.code === "40001"
        ? t("O atendimento mudou. Atualize e tente novamente.")
        : error.message,
