...
import { criarFunil } from "@/lib/pipelines/pipeline-operations";
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const supabase = await createClient();
  try {
    const { pipelineId: _pipelineId, ...body } = await criarFunil(
      {
        supabase,
    ...
    return ok(body, { status: 201, requestId });
  } catch (err) {
    if (err instanceof ApiError) {
      return fail(err.code === "validation_error" ? "unprocessable_entity" : err.code, err.message, err.status, {
        requestId,
        details: err.details,
      });
    }
    return fail("internal_error", err instanceof Error ? err.message : "Falha ao criar funil.", 500, { requestId });
  }
}
