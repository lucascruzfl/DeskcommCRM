...
export async function POST(req: Request) {
  ...
  const parsed = z
    .object({ connection_id: z.uuid() })
    .strict()
    .safeParse(await req.json().catch(() => null));
  if (!parsed.success)
    return fail("validation_failed", "Confira a conexão escolhida.", 422, { requestId });
  ...
  try {
    ...
    void audit({
      action: "agenda.google_catalog_updated",
    ...
    return fail(
      "conflict",
      "Não foi possível atualizar. Confira a conexão e tente novamente.",
      409,
      { requestId },
    );
  }
