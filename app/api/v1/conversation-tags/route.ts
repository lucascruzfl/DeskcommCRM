...
export async function GET(_req: NextRequest): Promise<Response> {
  ...
  const raw = (data?.settings as Record<string, unknown> | null)?.["canonical_conversation_tags"];
  ...
  const { data: emUso, error: erroEmUso } = await supabase.rpc("fn_tags_de_conversa_em_uso", {
  ...
  const tags = [...new Set([...canonicas, ...usadas])].sort((a, b) => a.localeCompare(b, "pt-BR"));
