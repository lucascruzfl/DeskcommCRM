...
export async function criarPipeline(
  ...
  if (error || !data)
    throw new ApiError(
      ...
      error?.code === "23505" ? "state_conflict" : "internal_error",
      undefined,
      ctx.requestId,
      error?.message,
    );
  ...
  await audit({
    ...
    metadata: { ...a.metadata, name, slug: row.slug, is_default: row.is_default },
  });
...
export async function atualizarPipeline(
  ctx: PipelineOperationContext,
  pipelineId: string,
  input: AtualizarPipelineInput,
) {
  const funis = await lerFunis(ctx.supabase, ctx.organizationId);
  const alvo = funis.find((f) => f.id === pipelineId);
  if (!alvo)
    throw new ApiError(404, "not_found", undefined, ctx.requestId, "Funil não encontrado.");
  if (alvo.is_archived && input.is_archived !== false)
    throw new ApiError(
      409,
      "state_conflict",
      undefined,
      ctx.requestId,
      "Tire o funil do arquivo antes de editá-lo.",
    );
  if (
    alvo.is_archived &&
    ...
    throw new ApiError(
      ...
      undefined,
      ctx.requestId,
      "Tire o funil do arquivo antes de editar outros campos.",
    );
  }
  ...
  if (input.name !== undefined) {
    ...
    if (!verdict.ok)
      throw new ApiError(422, "unprocessable_entity", undefined, ctx.requestId, verdict.erro);
    patch.name = name;
  }
  ...
  if (input.after_pipeline_id !== undefined) {
    ...
    const index =
      input.after_pipeline_id === null
        ? -1
        : ativos.findIndex((f) => f.id === input.after_pipeline_id);
    if (input.after_pipeline_id !== null && index < 0)
      throw new ApiError(
        422,
        "unprocessable_entity",
        undefined,
        ctx.requestId,
        "Funil vizinho não encontrado nesta organização.",
      );
    const position = posicaoEntre(
      ativos[index]?.position ?? null,
      ativos[index + 1]?.position ?? null,
    );
    if (!Number.isFinite(position))
      throw new ApiError(
        ...
        undefined,
        ctx.requestId,
        "A ordenação precisa ser recarregada.",
      );
  ...
  if (Object.keys(patch).length) {
    ...
    if (found) Object.assign(found.patch, patch);
    else updates.push({ pipelineId, patch });
  }
  if (!updates.length)
    throw new ApiError(422, "unprocessable_entity", undefined, ctx.requestId, "Nada para alterar.");
  ...
  const a = actorAudit(ctx);
  await audit({
    action: input.is_archived === false ? "pipeline.unarchived" : "pipeline.updated",
    organizationId: ctx.organizationId,
    actorUserId: a.actorUserId,
    actorApiTokenId: a.actorApiTokenId,
    resourceType: "crm_pipeline",
...
export async function arquivarPipeline(ctx: PipelineOperationContext, pipelineId: string) {
  ...
  await audit({
    ...
    resourceId: pipelineId,
    requestId: ctx.requestId,
    metadata: { ...a.metadata, name: alvo.name, leads: deps.negocios },
  });
...
export async function atualizarConfiguracaoDoPipeline(
  ...
  const { data: row, error: readError } = await ctx.supabase
    .from("crm_pipelines")
    .select("vocabulary, settings")
    .eq("organization_id", ctx.organizationId)
    .eq("id", pipelineId)
    .maybeSingle();
  if (readError)
    throw new ApiError(500, "internal_error", undefined, ctx.requestId, readError.message);
  if (!row) throw new ApiError(404, "not_found", undefined, ctx.requestId, "Funil não encontrado.");
  const vocabulary = patch.vocabulary
    ? { ...((row.vocabulary as Record<string, unknown> | null) ?? {}), ...patch.vocabulary }
    : row.vocabulary;
