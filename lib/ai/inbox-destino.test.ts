...
const aviso = (
  kind = "handoff",
  ref_kind: string | null = "conversation",
  ref_id: string | null = ID,
...
function leitor(rows = [ID], falha = false, pipelineVisible = true) {
  ...
  const client = {
    from(table: string) {
      const q = { table } as (typeof queries)[number];
      queries.push(q);
      const chain = {
        select: () => chain,
        eq: (key: string, value: string) => {
          expect(key).toBe("organization_id");
          q.org = value;
          return chain;
        },
            ...
            data:
              table === "operational_crm_pipelines"
                ? pipelineVisible
                  ? [{ id: PIPELINE }]
                  : []
                : rows.map((id) => ({ id, pipeline_id: PIPELINE })),
            error: falha ? { message: "SECRET payload" } : null,
          }).then(resolve),
      };
      return chain;
    },
...
describe("destinos da Central", () => {
  it("aviso Meet abre só compromisso visível ao agente e mantém os vetos do resolvedor", async () => {
    for (const [role, rows, failure, state] of [
      ...
      const [item] = await resolverDestinosDosAvisos(l.client, ORG, role, [
        aviso("other", "appointment"),
      ]);
      expect(item?.destination.estado).toBe(state);
      if (state === "disponivel")
        expect(item?.destination).toMatchObject({
          href: `/app/agenda?compromisso=${ID}`,
          rotulo: "Abrir compromisso",
        });
      else expect(item?.destination).not.toHaveProperty("href");
      expect(l.queries).toEqual(
        role === "viewer" ? [] : [{ table: "calendar_appointments", org: ORG, ids: [ID] }],
      );
    }
    const l = leitor();
    const [invalidPair] = await resolverDestinosDosAvisos(l.client, ORG, "admin", [
      aviso("handoff", "appointment"),
    ]);
    expect(invalidPair?.destination).not.toHaveProperty("href");
    expect(l.queries).toHaveLength(0);
  });
  it("toda categoria possui política", () =>
    expect(Object.keys(POLITICAS_DE_AVISO).sort()).toEqual(Object.keys(KIND_LABEL).sort()));
  it.each([
    ["handoff", "conversation", `/app/inbox/${ID}`],
    ["job_dead", "conversation", `/app/inbox/${ID}`],
    ["handoff", "contact", `/app/contacts/${ID}`],
    ["other", "lead", `/app/pipelines/${PIPELINE}?lead=${ID}`],
    ["followup_dead", "followup_enrollment", `/app/ai/followups/enrollments/${ID}`],
    ["qr_rescan", "channel_session", "/app/connections"],
    ["conhecimento_nao_indexado", "ai_knowledge_source", "/app/ai/knowledge/sources"],
  ])("%s/%s abre somente contexto real", async (kind, ref, href) => {
    const l = leitor();
    const [item] = await resolverDestinosDosAvisos(l.client, ORG, "admin", [aviso(kind, ref)]);
  ...
  it("consulta por tipo e deduplica IDs, não N+1", async () => {
    const l = leitor();
    await resolverDestinosDosAvisos(l.client, ORG, "agent", [
      ...Array.from({ length: 150 }, () => aviso()),
      aviso("handoff", "contact"),
    ]);
    expect(l.queries).toEqual([
  ...
  it("manager recebe uso/acervo e agent apenas orientação", async () => {
    for (const role of ["agent", "manager"] as const) {
      const items = await resolverDestinosDosAvisos(leitor().client, ORG, role, [
        aviso("budget_warning", "ai_budget", ORG),
        aviso("conhecimento_nao_indexado", "ai_knowledge_source"),
      ]);
      expect(items.map((i) => i.destination.estado)).toEqual(
        Array(2).fill(role === "agent" ? "sem_permissao" : "disponivel"),
      );
    }
  });
  it.each([null, "not-a-uuid", missing])("ref %s não vira URL", async (id) => {
    const [item] = await resolverDestinosDosAvisos(leitor().client, ORG, "agent", [
      aviso("handoff", "conversation", id),
    ]);
    expect(item?.destination.estado).toBe("indisponivel");
  ...
  it("zero linhas RLS, removido e erro são indistinguíveis; diagnóstico sanitizado", async () => {
    ...
    const error = await resolverDestinosDosAvisos(leitor([ID], true).client, ORG, "agent", [
      aviso(),
    ]);
  ...
  it.each(["budget_exceeded", "risk_backlog_seeded", "reactivation_expired"])(
    "%s aceita apenas org ativa",
    async (kind) => {
      const ref = kind === "budget_exceeded" ? "ai_budget" : "organization";
      const items = await resolverDestinosDosAvisos(leitor().client, ORG, "admin", [
        aviso(kind, ref, ORG),
        aviso(kind, ref, ID),
      ]);
      expect(items[0]?.destination.estado).toBe("disponivel");
  ...
  it("kind/ref desconhecidos, pares inválidos e URLs arbitrárias falham fechados", async () => {
    const l = leitor();
    const items = await resolverDestinosDosAvisos(l.client, ORG, "admin", [
      aviso("novo"),
      aviso("handoff", "https://evil.test"),
      aviso("handoff", "channel_session"),
      aviso("__proto__"),
      aviso("other", "__proto__"),
    ]);
    expect(items.every((i) => !("href" in i.destination))).toBe(true);
    expect(l.queries).toHaveLength(0);
  });
  it("referências técnicas não inventam tela; modelos sem ID abrem canal Parceiro", async () => {
    const items = await resolverDestinosDosAvisos(leitor().client, ORG, "admin", [
      aviso("job_dead", "job_queue"),
      aviso("channel_template_review", null, null),
      aviso("contact_proposal_expired", "organization", ORG),
    ]);
    expect(items[0]?.destination.estado).toBe("sem_destino");
    expect(items[1]?.destination).toMatchObject({
      href: "/app/connections?aba=parceiro&sub=templates",
      orientacao: expect.stringContaining("não identifica"),
    });
    expect(items[2]?.destination.estado).toBe("sem_destino");
  });
  it("envio preso representa uma conversa, não todas", async () => {
    const [item] = await resolverDestinosDosAvisos(leitor().client, ORG, "agent", [
      aviso("message_send_stuck"),
    ]);
  ...
  it("textos dinâmicos do catálogo e fallbacks têm espanhol", async () => {
    ...
    for (const [kind, p] of Object.entries(POLITICAS_DE_AVISO)) {
      ...
      for (const papel of ["agent", "admin"] as const) {
        const items = await resolverDestinosDosAvisos(leitor().client, ORG, papel, [
          aviso(kind, null, null),
          aviso(kind, "conversation", missing),
          ...p.refs.map((ref) =>
            aviso(kind, ref, ref === "organization" || ref === "ai_budget" ? ORG : ID),
          ),
        ]);
    ...
    const [unknown] = await resolverDestinosDosAvisos(leitor().client, ORG, "agent", [
      aviso("new-kind"),
    ]);
