...
interface Alvo {
  ...
  papel: Role;
  rotulo: string;
  href: (id: string, pipelineId?: string) => string;
  ativo?: boolean;
}
...
export const REFERENCIAS_DE_AVISO = {
  ai_agent: {
    tabela: "ai_agents",
    papel: "admin",
    rotulo: "Revisar agente",
    href: (id: string) => `/app/ai/agents/${id}`,
    ativo: true,
  },
  appointment: {
    tabela: "calendar_appointments",
    papel: "agent",
    rotulo: "Abrir compromisso",
    href: (id: string) => `/app/agenda?compromisso=${id}`,
  },
  conversation: {
    tabela: "conversations",
    papel: "agent",
    rotulo: "Abrir conversa",
    href: (id: string) => `/app/inbox/${id}`,
  },
  contact: {
    tabela: "contacts",
    papel: "agent",
    rotulo: "Ver contato",
    href: (id: string) => `/app/contacts/${id}`,
  },
  lead: {
    tabela: "crm_leads",
    papel: "agent",
    rotulo: "Abrir negócio",
    href: (id: string, pipelineId?: string) => `/app/pipelines/${pipelineId}?lead=${id}`,
  },
  followup_enrollment: {
    tabela: "followup_enrollments",
    papel: "viewer",
    rotulo: "Abrir acompanhamento",
    href: (id: string) => `/app/ai/followups/enrollments/${id}`,
  },
  channel_session: {
    tabela: "channel_sessions",
    papel: "admin",
    rotulo: "Revisar conexão",
  ...
  ai_knowledge_source: {
    ...
    papel: "manager",
    rotulo: "Abrir base de conhecimento",
  ...
  agent_case: {
    ...
    href: (id: string) => `/app/ai/cases?caso=${id}`,
  },
  ...
  followup_flow: {
    ...
    papel: "manager",
    rotulo: "Abrir o fluxo",
    href: (id: string) => `/app/ai/followups/${id}`,
  },
} satisfies Record<string, Alvo>;

export type InboxRefKind =
  keyof typeof REFERENCIAS_DE_AVISO | "organization" | "ai_budget" | "job_queue" | "cron_jobs";
type ContextoGeral = { papel: Role; href: string; rotulo: string };
interface Politica {
  refs: readonly InboxRefKind[];
  orientacao: string;
  geral?: ContextoGeral;
}
const EVOLUCAO: ContextoGeral = {
  papel: "manager",
  href: "/app/ai/evolution",
  rotulo: "Abrir evolução do assistente",
};
const CONEXOES: ContextoGeral = {
  papel: "admin",
  href: "/app/connections",
  rotulo: "Revisar conexões",
};
...
export const POLITICAS_DE_AVISO = {
  ...
  appointment_outcome_required: {
    refs: ["appointment"],
    orientacao: "Abra o compromisso e confirme a presença.",
  },
  appointment_recovery_review: {
    refs: ["appointment"],
    orientacao: "Confira o motivo e escolha o próximo passo no compromisso.",
  },
  routing_unassigned: {
    refs: ["conversation"],
    orientacao: "Confira os responsáveis em Configurações → Atendimento.",
  },
  qr_rescan: {
    refs: ["channel_session"],
    orientacao: "Peça a quem administra para revisar a conexão do WhatsApp.",
  },
  job_dead: {
    refs: ["conversation", "job_queue", "cron_jobs"],
    orientacao:
      "Confira o motivo deste aviso com quem administra antes de tentar a operação novamente.",
  },
  event_dead: {
    refs: [],
    orientacao: "Peça a quem administra para conferir o processamento descrito neste aviso.",
  },
  budget_exceeded: {
    refs: ["ai_budget"],
    orientacao: "Peça ao gestor para revisar o limite e o uso de IA.",
  },
  budget_warning: {
    refs: ["ai_budget"],
    orientacao: "Peça ao gestor para revisar o limite e o uso de IA.",
  ...
  followup_dead: {
    refs: ["followup_enrollment"],
    orientacao: "Peça ao gestor para revisar o acompanhamento que parou.",
  },
  snooze_expired: {
    refs: ["conversation"],
    orientacao: "Confira se cabe retomar o atendimento descrito neste aviso.",
  },
  next_action_ambiguous: {
    refs: ["contact"],
    orientacao: "Confira os negócios do contato e escolha a qual deles pertence a próxima ação.",
  },
  risk_backlog_seeded: {
    refs: ["organization"],
    orientacao: "Revise os negócios parados no Radar e defina o próximo passo.",
  },
  reactivation_expired: {
    refs: ["organization"],
    orientacao: "Revise no Radar se ainda cabe retomar os negócios indicados.",
  },
  ...
  message_send_stuck: {
    refs: ["conversation"],
    orientacao: "Confira a resposta que não chegou antes de decidir se precisa enviar novamente.",
  },
  midia_nao_lida: {
    refs: [],
    orientacao:
      "Peça ao gestor para revisar o provedor e as credenciais de leitura de fotos e áudios.",
    geral: { papel: "manager", href: "/app/ai/providers", rotulo: "Revisar provedores de IA" },
  },
  channel_template_review: {
    ...
    geral: {
      papel: "admin",
      href: "/app/connections?aba=parceiro&sub=templates",
      rotulo: "Revisar modelos do canal",
    },
  },
  channel_number_alert: {
    refs: ["channel_session"],
    orientacao: "Peça a quem administra para revisar a situação do número nas conexões.",
    geral: CONEXOES,
  },
  ...
  canal_mudo_sem_numero: {
    refs: ["channel_session"],
    orientacao:
      "Peça a quem administra para autorizar os números de teste em Conexões ou abrir o canal ao público.",
  },
  promise_unfulfilled: {
    refs: ["conversation"],
    orientacao: "Confira o compromisso descrito e defina quem fica responsável.",
  },
  ...
  voice_call_missed: {
    refs: ["contact"],
    orientacao: "Retorne a ligação quando puder — quem ligou não foi atendido.",
  },
  ...
  other: {
    refs: ["lead", "channel_session", "appointment", "ai_agent"],
...
const SEM_DESTINO: DestinoDoAviso = {
  estado: "sem_destino",
  orientacao: "Este aviso não tem um contexto que possa ser aberto nesta versão.",
};
const INDISPONIVEL: DestinoDoAviso = {
  estado: "indisponivel",
  orientacao:
...
function politica(item: ReferenciaDoAviso): Politica | undefined {
  return Object.hasOwn(POLITICAS_DE_AVISO, item.kind)
    ? POLITICAS_DE_AVISO[item.kind as InboxKind]
    : undefined;
}
function alvo(ref: string | null): Alvo | undefined {
  return ref && Object.hasOwn(REFERENCIAS_DE_AVISO, ref)
    ? REFERENCIAS_DE_AVISO[ref as keyof typeof REFERENCIAS_DE_AVISO]
    : undefined;
}
function permite(papel: Role, minimo: Role) {
  return ROLE_RANK[papel] >= ROLE_RANK[minimo];
}
...
export async function resolverDestinosDosAvisos<T extends ReferenciaDoAviso>(
  ...
  for (const item of itens) {
    const p = politica(item),
      a = alvo(item.ref_kind);
    if (
      !p ||
      !a ||
      !p.refs.includes(item.ref_kind as InboxRefKind) ||
      !permite(papel, a.papel) ||
      !uuid.safeParse(item.ref_id).success
    )
      continue;
  ...
  await Promise.all(
    [...grupos].map(async ([ref, ids]) => {
      const a = alvo(ref)!;
      try {
        let query = leitor
          .from(a.tabela)
          .select(ref === "lead" ? "id, pipeline_id" : "id")
        ...
        if (ref === "lead") {
          ...
          const idsDeFunis = [
            ...new Set(leads.map((l) => l.pipeline_id).filter((id) => uuid.safeParse(id).success)),
          ];
          if (idsDeFunis.length > 0) {
            ...
            const { data: funis, error: erroFunis } = await leitor
            ...
            for (const lead of leads)
              if (permitidos.has(lead.pipeline_id)) funilPorLead.set(lead.id, lead.pipeline_id);
          }
          visiveis.set(ref, new Set(funilPorLead.keys()));
        } else
          visiveis.set(
            ref,
            new Set(((data ?? []) as unknown as Array<{ id: string }>).map((linha) => linha.id)),
          );
      } catch {
        // Somente catálogo fechado e contagem. Nunca erro bruto, título, UUID ou payload.
        logger.warn("[inbox] contexto indisponível na projeção", {
          referencia: ref,
          quantidade: ids.size,
        });
      }
  ...
  return itens.map((item) => {
    ...
    if (p) {
      if (item.ref_kind === null && item.ref_id === null) {
        destination = p.geral
          ? permite(papel, p.geral.papel)
            ? {
                estado: "disponivel",
                href: p.geral.href,
                rotulo: p.geral.rotulo,
                orientacao: p.orientacao,
              }
            : semPermissao(p.geral.papel)
        ...
        if (a) {
          destination = !permite(papel, a.papel)
            ? semPermissao(a.papel)
            : visiveis.get(item.ref_kind!)?.has(item.ref_id!)
              ? {
                  estado: "disponivel",
                  rotulo: ROTULO_POR_KIND[item.kind] ?? a.rotulo,
                  href: a.href(item.ref_id!, funilPorLead.get(item.ref_id!)),
                }
              : INDISPONIVEL;
        } else if (item.ref_kind === "ai_budget" || item.ref_kind === "organization") {
          destination =
            item.ref_id !== organizationId
              ? INDISPONIVEL
              : item.kind === "contact_proposal_expired"
                ? { estado: "sem_destino", orientacao: p.orientacao }
                : !permite(papel, item.ref_kind === "ai_budget" ? "manager" : "agent")
                  ? semPermissao("manager")
                  : {
                      estado: "disponivel",
                      href: item.ref_kind === "ai_budget" ? "/app/ai/usage" : "/app/radar",
                      rotulo: item.ref_kind === "ai_budget" ? "Abrir uso de IA" : "Abrir Radar",
                    };
