...
const shape = {
  ...
  pipeline_id: z
    ...
    .uuid()
    .optional()
    .describe("Obrigatório para contato; no lead o funil é resolvido pelo próprio recurso."),
...
export const crmSetCustomFieldValues: McpToolDefinition<typeof shape> = {
  ...
  handler: async (input, ctx) => {
    ...
    if (input.target_kind === "lead") {
      ...
      if (!pipelineId)
        throw new ApiError(
          ...
          undefined,
          ctx.requestId,
          "pipeline_id é obrigatório para contato.",
        );
      const contact = await getContactHandler(
        ctx.supabase,
        { organization_id: ctx.organizationId, actor: ctx.actor, requestId: ctx.requestId },
        { contactId: input.target_id, decryptPurpose: null },
      );
    ...
    try {
      patch = validarValoresDeCampos(
        pipeline.settings as Record<string, unknown> | null,
        input.values,
        input.clear_keys,
      );
    } catch (error_) {
      throw new ApiError(
        ...
        undefined,
        ctx.requestId,
        error_ instanceof Error ? error_.message : "Valor inválido.",
      );
    }
    if (input.target_kind === "lead") {
