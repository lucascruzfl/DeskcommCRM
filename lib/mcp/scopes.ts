...
export const MCP_DOMAINS = [
  "agents", "ai", "appointments", "automations", "channels", "contacts",
  "conversations", "followups", "knowledge", "leads", "messages", "pipelines",
  "products", "routing", "team", "templates", "webhooks",
] as const satisfies ReadonlyArray<McpToolDomain>;

export const MCP_CAPABILITIES = [
  "agent_activation", "agent_publication", "automation_activation",
  "destructive_operations", "human_handoff", "send_messages",
...
export function isKnownApiTokenScope(scope: string): boolean {
  ...
  if (MCP_DOMAINS.some((domain) => scope === `${domain}:read` || scope === `${domain}:write`)) return true;
