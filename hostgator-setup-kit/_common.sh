...
gravar_imagens() {
  ...
  if [ "${DESKCOMM_UPDATE_CHANNEL:-official}" = custom-mcp ]; then
    [ -n "${MCP_APP_PIN:-}" ] && [ -n "${MCP_WORKER_PIN:-}" ] \
      && [ -n "${MCP_SCHEDULER_PIN:-}" ] && [ -n "${MCP_VOICE_PIN:-}" ] \
      || { echo 'Canal MCP sem os quatro digests validados.' >&2; return 1; }
    # O mesmo namespace alimenta app, worker, scheduler e voz.
    IMG_NS="${DESKCOMM_IMAGE_REPOSITORY%/*}"
    IMG_APP="${IMG_NS}/deskcommcrm"
    IMG_WORKER="${IMG_NS}/deskcomm-worker"
    IMG_SCHEDULER="${IMG_NS}/deskcomm-scheduler"
    IMG_VOICE_AGENT="${IMG_NS}/deskcomm-voice-agent"
  ...
  set_env_var "$envfile" APP_IMAGE             "${MCP_APP_PIN:-${IMG_APP}:${versao}}"
  set_env_var "$envfile" APP_PULL_POLICY       "$politica"
  set_env_var "$envfile" WORKER_IMAGE          "${MCP_WORKER_PIN:-${IMG_WORKER}:${versao}}"
  set_env_var "$envfile" WORKER_PULL_POLICY    "$politica"
  set_env_var "$envfile" SCHEDULER_IMAGE       "${MCP_SCHEDULER_PIN:-${IMG_SCHEDULER}:${versao}}"
  ...
  set_env_var "$envfile" VOICE_AGENT_IMAGE       "${MCP_VOICE_PIN:-${IMG_VOICE_AGENT}:${versao}}"
