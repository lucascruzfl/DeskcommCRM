...
| Skills de IA | versões e restauração | A | falta leitura de versões e restauração org-scoped no MCP; a restauração usa `setSkillPointer` e exige manager |
...
Esta tabela é triagem, **não** certificação de cobertura. Há **ao menos 3 gaps A conhecidos**: duplicar fluxo de follow-up, listar versões de uma skill e restaurar versão. O total de gaps A da 1.45.0 ainda é **indeterminado** até a auditoria por operação. `RELEASE-AUDIT.json` continua em
