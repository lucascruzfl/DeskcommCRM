...
function makeSupabase(botSilencedUntil: string | null, snapshot?: () => Record<string, unknown>) {
  ...
  const client = {
    from(table: string) {
      if (table === "operational_conversations") {
        const q = { eq: () => q, maybeSingle: async () => ({ data: { id: CONV }, error: null }) };
        return {
          select: () => q,
          update: (patch: Row) => {
            patches.push(patch);
      ...
      if (["organizations", "operational_organizations"].includes(table)) {
        const q = {
          select: () => q,
          eq: () => q,
      ...
      if (["messages", "operational_messages"].includes(table)) {
        return {
          ...
          update: (patch: Row) => ({
            eq: () => ({
              select: () => ({
                maybeSingle: async () => ({ data: { id: "msg-1", ...patch }, error: null }),
              }),
            }),
          }),
        };
...
describe("sendMessageHandler — silêncio da IA de 5min após resposta manual humana", () => {
  ...
  it("handoff permanente (infinity) já ativo → resposta manual NÃO encurta para 5min", async () => {
    ...
    const convPatch = patches[patches.length - 1]!;
    expect(
      convPatch.bot_silenced_until,
      "rebaixou o handoff permanente para uma janela de 5min",
    ).toBeUndefined();
  });
...
it("sink recusa close/reopen ocorrido depois da primeira leitura e receiver recebe zero", async () => {
  ...
  const receiver = createServer((req, res) => {
    ...
    res.end(JSON.stringify({ id: { id: "REAL1" } }));
  });
  ...
  try {
    ...
    for (const changed of [
      { status: "closed", service_revision: 2 },
      { status: "open", service_revision: 3 },
    ]) {
      let reads = 0;
      ...
      await expect(sendMessageHandler(supabase, ctx, input)).rejects.toThrow(
    ...
    let reads = 0;
    const late = makeSupabase(null, () => ({
      ...boundary,
      status: "open",
      demanda_fechada_em: null,
      service_revision: reads++ < 2 ? 1 : 3,
    }));
    await expect(sendMessageHandler(late.supabase, ctx, input)).rejects.toThrow(
      "service_boundary_stale",
    );
    expect(reads).toBe(3);
...
it("ação de automação conserva origem durante pacing e dois envios vigentes chegam ao receiver", async () => {
  ...
  const receiver = createServer((req, res) => {
    hits.push(req.url ?? "");
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({ id: { id: "ACTION" } }));
  });
  await new Promise<void>((resolve) => receiver.listen(0, "127.0.0.1", resolve));
  try {
    const address = receiver.address();
    ...
    const ctx = {
      ...
      event: {
        ...
        payload: { service_origin: { kind: "continuation", boundary } },
      },
      context: { contact: { id: CONTACT, phone_number: "+5531999998888" } },
    ...
    generation.run = async () => {
      revision = 3;
      return { ok: true, texto: "Resposta gerada" };
    };
    ...
    expect(hits.filter((url) => url.endsWith("/sendText"))).toHaveLength(2);
    revision = 1;
    generation.run = async () => ({ ok: true, texto: "Resposta gerada" });
    ...
    const aviso = await avisarLeadDoCrm(supabase, {
      organizationId: ORG,
      contactId: CONTACT,
      conversationId: CONV,
    ...
    const antigo = await avisarLeadDoCrm(supabase, {
      organizationId: ORG,
      contactId: CONTACT,
      conversationId: CONV,
      reason: "Caso antigo",
