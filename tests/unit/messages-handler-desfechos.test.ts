    ...
    from(table: string) {
      if (table === "channel_sessions") {
        const query = {
          ...
          eq: () => query,
          maybeSingle: async () => ({
            data: { metadata: opts.channelMetadata ?? {} },
            error: null,
          }),
        };
        return query;
      ...
      if (["messages", "operational_messages"].includes(table)) {
        return {
          insert: (row: Row) => {
            ...
            return {
              select: () => ({ single: async () => ({ data: { ...state.message }, error: null }) }),
            };
          },
          update: (patch: Row) => {
...
describe("sendMessageHandler — os 6 desfechos do envio", () => {
  it("revalida a lista no sink, inclusive para automação, sem transformar teste em opt-out", async () => {
    ...
    for (const actor of [
      { type: "ai_agent", id: USER, role: "agent" },
      { type: "webhook_source", id: USER },
    ] as const) {
  ...
  it("número autorizado passa pelo gate; resposta humana não depende da lista", async () => {
    ...
    const channelMetadata = {
      ai_gate: "allowlist",
      ai_gate_mode: "pre_go_live",
      ai_test_phone_numbers: ["+5531999998888"],
    };
    const tester = await sendMessageHandler(
      makeSupabase(conversationRow(), null, { channelMetadata }),
      { ...ctx, actor: { type: "ai_agent", id: USER, role: "agent" } },
      textInput(),
    );
    expect(tester.status).toBe("queued");
    const human = await sendMessageHandler(
      makeSupabase(conversationRow(), null, {
  ...
  it("3. sessão fora de WORKING: fica queued com channel_session_not_working", async () => {
    ...
    expect(msg.status).toBe("queued");
    expect((msg.metadata as Record<string, unknown>).queued_reason).toBe(
  ...
  it("4. com media_storage_path: sent + external_id + ack 0, pelo endpoint de mídia", async () => {
    ...
    const fetchMock = vi.fn(async (..._args: unknown[]) =>
      Response.json({ id: { _serialized: "MEDIA1" } }),
    );
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("5. texto puro: sent + external_id + ack 0, pelo endpoint de texto", async () => {
    ...
    expect(msg.ack).toBe(0);
    expect(msg.error_code).toBeNull();
    const sendText = fetchMock.mock.calls.find(
      ([url]) => String(url) === `${WAHA_BASE}/api/sendText`,
    );
...
describe("sendMessageHandler — token de servidor (api_token) no ponto de uso", () => {
  ...
  it("grava sent_by_user_id = null: o id do TOKEN não vai para a coluna com FK para auth.users", async () => {
    ...
    expect(tokenDeServidor.type, "o caso perdeu o alvo: o ator já não é api_token").toBe(
      "api_token",
    );
    expect(
      msg.sent_by_user_id,
  ...
  it("consulta o modo de teste do canal: número fora da lista não recebe, e nada sai pela rede", async () => {
    wahaConfigured(true);
    const fetchMock = vi.fn(async (..._args: unknown[]) =>
      Response.json({ key: { id: "NAO-DEVIA-SAIR" } }),
    );
    vi.stubGlobal("fetch", fetchMock);
