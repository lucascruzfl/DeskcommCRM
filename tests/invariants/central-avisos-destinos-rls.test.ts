...
beforeAll(() => {
  sql(
    `insert into auth.users (id, email) values ('${actor}', 'avisos-a@invariant.test'), ('${other}', 'avisos-b@invariant.test');`,
  );
  for (const organization of [org, external]) {
    ...
    for (const i of organization === org ? [0, 1, 2] : [3]) {
      ...
      const owner = i === 0 || i === 3 ? `'${actor}'` : i === 1 ? "null" : `'${other}'`;
...
function authenticated() {
  return {
    from(table: string) {
      expect(["conversations", "crm_leads", "operational_crm_pipelines"]).toContain(table);
      let orgFilter = "",
        ids: string[] = [];
      let columns = "id";
      const chain = {
...
describe("destinos Central respeitam RLS de registro e organização", () => {
  it.each(["own", "own_and_unassigned"])(
    "agent em %s: própria sim, outro dono não, fila conforme RLS",
    async (mode) => {
      sql(
        `update organizations set settings = jsonb_build_object('visibility_mode', '${mode}') where id = '${org}';`,
      );
      ...
      const expected = [
        "disponivel",
        mode === "own" ? "indisponivel" : "disponivel",
        "indisponivel",
        "indisponivel",
      ];
